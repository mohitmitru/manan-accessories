...
export default function AdminDashboard() {
  ...
  const saveProduct = async (event) => {
    ...
    const discount = mrp > sellingPrice ? Math.min(95, Math.round(((mrp - sellingPrice) / mrp) * 100)) : 0;
  ...
  const editProduct = (item) => {
    ...
    setProduct({
      name: item.name,
      price: item.salePrice || Math.round(Number(item.price || 0) - (Number(item.price || 0) * Number(item.discount || 0)) / 100),
      mrp: item.price,
  ...
  const moveProduct = async (item, direction) => {
    const currentIndex = products.findIndex((productItem) => productItem._id === item._id);
    ...
    try {
      await api.patch("/products/order", { ids: nextProducts.map((productItem) => productItem._id) });
      await loadData();
    } catch (error) {
      alert(error.response?.data?.message || "Product order save failed.");
      await loadData();
    }
  };
    ...
    <main className="admin-shell">
      <aside className="admin-sidebar">
        ...
        <div className="admin-menu">
          ...
          <button className={activeView === "nameListing" ? "active" : ""} onClick={() => setActiveView("nameListing")}><ListOrdered size={18} /> Listing</button>
      ...
      <section className="admin-main">
        ...
        {activeView === "products" && <div className="admin-grid single-panel">
          <form className="panel" onSubmit={saveProduct}>
            ...
            <label className="upload-label product-image-upload">Product images
              <span className="file-picker-row">
                <span className="file-button">Choose Files</span>
                <span className="file-status">{images.length ? `${images.length} image(s) selected` : "No file chosen"}</span>
              </span>
              <input className="hidden-file-input" type="file" accept="image/*" multiple onChange={(e) => setImages(Array.from(e.target.files || []))} />
        ...
        {activeView === "nameListing" && <section className="panel">
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Order</th><th>Image</th><th>Name</th><th>Category</th><th>Price</th><th>Stock</th><th>Hero Slider</th><th>Actions</th></tr></thead>
              <tbody>
                {visibleProducts.map((item) => (
                  <tr key={item._id}>
                    <td>
                      <button className="mini" onClick={() => moveProduct(item, -1)} aria-label="Move product up"><ArrowUp size={15} /></button>
                      <button className="mini" onClick={() => moveProduct(item, 1)} aria-label="Move product down"><ArrowDown size={15} /></button>
                    </td>
                    ...
                    <td>
                      ...
                      <button className="mini danger" onClick={() => deleteProduct(item)}><Trash2 size={15} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
